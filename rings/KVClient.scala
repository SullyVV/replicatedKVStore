...
class RouteMsg(var operation: Int, var key: BigInt, var value: Int, var versionNum: Long)
/**
...
//  }
  ...
  def directRead(key: BigInt): ReturnData = {
    ...
    val randomStore = generator.nextInt(10)
    val hashedKey = hashForKey(key).toInt
    val future = ask(stores(randomStore), RouteMsg(0, key, hashedKey, -1, -1))
  ...
  def directWrite(key: BigInt, value: Int): Boolean = {
    ...
    val randomStore = generator.nextInt(10)
    val hashedKey = hashForKey(key).toInt
    val future = ask(stores(randomStore), RouteMsg(1, key, hashedKey, value, versionNum))
    val done = Await.result(future, timeout.duration).asInstanceOf[Boolean]
    return done
  }
